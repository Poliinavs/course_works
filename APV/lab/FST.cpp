...
namespace FST
{
	...
	bool execute(FST& fst)
	{
		...
		fst.rstates = new short[1]; //// возможные состояния автомата на данной позиции
		fst.rstates[0] = 0;

		for (short i = 0; i < strlen(str); i++)
		{
			symb = str[i];
			fst.position++; // текущая позиция в цепочке

			if (isAllowed(symb, fst.rstates, nRStates, fst))//проверил что сивол пожходи какому-то сиволу из задан сост, доб нов сост
			{
				fst.rstates = setRelState(symb, fst, &nRStates, fst.rstates);  //rstates возможные состояния автомата на данной позиции
	...
	)
	{
		for (short i = 0; i < length; i++)
		{
			for (short j = 0; j < fst.nodes[rstates[i]].n_relation; j++) //nodes-граФ ПЕРЕходов.n_relation колич индентичн реб
			{
				if (symb == fst.nodes[rstates[i]].relations[j].symbol) //прозодимся по массиву сивмолов на сост если символ совпад с каким симвл ntru
				{
					return true;
				}
			}
