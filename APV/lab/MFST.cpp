...
namespace MFST
{
	...
	bool Mfst::start(Log::LOG log)
	{
		...
		case NS_NORULE:
			MFST_TRACE4("------>NS_NORULE", *Mfst::pars)
				...
				* Mfst::pars << "------------------------------------------------------------------------------------------   ------" << std::endl;
			*Mfst::pars << getDiagnosis(0, buf,log) << std::endl;
			str = getDiagnosis(0, buf, log);
			*Mfst::pars << getDiagnosis(1, buf,log) << std::endl;
			str += getDiagnosis(1, buf, log);
			*Mfst::pars << getDiagnosis(2, buf,log) << std::endl;
			throw ERROR_THROW(801);
			exit(0);
			break;
	...
	char* Mfst::getDiagnosis(short n, char* buf, Log::LOG log)
	{
		...
		if (n < MFST_DIAGN_NUMBER && (lpos = diagnosis[n].lenta_position) >= 0)
		{
			...
			Error::ERROR err = Error::geterrorin(errid, lexTable.table[lpos].sn,lpos);
